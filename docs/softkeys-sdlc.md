...
**Status:** **Released — v1.2.1** (2026-07-27, Gate C2), and **Amendment A4 (F-22 English word prediction) in force per CR-21 — M13 implemented, pending Gate D1.** A3 (`Del` key) delivered in full per CR-20: M11 (F-21 + amended F-04, selftest 208/208) and M12 (package + GitHub Release). v1.2.0 released 2026-07-22; v1.1.0 and v1.0.0 released 2026-07-21. GitHub is the sole remote and canonical history per CR-17 + addendum (cgit archive deleted).
...
| 2026-07-22 | DF-02 | **Withdrawn — DF-02 closed.** Approver disposition: the reproduction is controlling evidence — No holds the default, Enter keeps settings, D-18 is satisfied as built. The screenshot read is attributed to Win11's hover-fill vs focus-ring distinction. No hardening: MB_YESNOCANCEL's third button judged worse than the theoretical gain. The T-15 re-run will include a deliberate Enter-press at the settings prompt as the witnessed confirmation. | Approved — Nasser |
| 2026-07-22 | — | **Gate B1 progress: T-14 passed.** Install + identity witnessed: no UAC prompt, per-user install, Start Menu entry, icon correct at all sizes. Checkbox question resolved with no DF-03: the first run showed both optional tasks unchecked (`Flags: unchecked`); the second run showed Inno's remembered selections from the uninstall registry key — by design. Remaining for Gate B1: T-15 re-run (with Enter-press check), T-16, T-17, T-18. | Approved — Nasser |
| 2026-07-22 | — | **Gate B1 approved — M9 complete.** Witnessed: T-15 both uninstall paths (Path A: Enter at the settings prompt kept settings — the DF-02 loop closed by the Approver's own keypress; full residue check clean. Path B: explicit Yes removed settings; clean). T-16 upgrade over a running instance (AppMutex close prompt, single Apps & Features entry, settings preserved, no duplicate shortcuts). T-17 portable exe (selftest 201/201 witnessed; Arabic caps, Alt+F4 chord on Win11 Notepad, capture toggle all green). T-18 (minimize/restore with focus never captured, typing landed; maximize still blocked; snap immunity at left/right/top edges; edge-drag resize functional after). Tagged `m9-icon-installer` (`219f182`). M10 authorized; shipping artifacts are the witnessed walkthrough build: exe `9df54a46…2c02`, setup `f4768823…56f3`. | Approved — Nasser |
| 2026-07-22 | — | **Gate B2 / M10 launch check passed.** SHA-256 match on both artifacts (exe `9df54a46…2c02`, setup `f4768823…56f3`) — the exact binaries witnessed through T-15–T-18, no rebuild between walkthrough and release; sanity launch clean. `main` + all tags pushed to origin; GitHub Release `v1.2.0` published with both assets, notes per B2.5 (A2 features, NF-09 statement, both hashes, SmartScreen/L-07 note, vboard credit). | Approved — Nasser |
| 2026-07-22 | — | **FINAL ACCEPTANCE v1.2.0.** Amendment A2 delivered in full: F-15 application icon (dual-script identity), F-16 per-user installer with zero elevation, F-17 clean uninstall with settings kept by default (confirmed by the Approver's Enter-press), F-18 in-place upgrade via mutex, F-19 minimize, F-20 snap immunity — witnessed via T-14–T-18, including one defect report (DF-02) investigated and correctly withdrawn on evidence. | **Signed off — Nasser** |
| 2026-07-27 | CR-20 | **Amendment A3 adopted — `Del` key delivered.** `docs/softkeys-v1.3-amendment.md` in force: **F-21** `Del` key (plain, 2 star units, cap label `Del`, internal name `Delete`, right end of Row 4 beside `↑`), **F-04 amended** (six-row wording — stale "five rows" since F-14 — and Row 4 now 32 star units; other main rows 30, function row 27); **D-21** placement, **D-22** scan code `0x53` **extended (E0 53)** with `Delete` added to the D-05 extended set, **D-23** name/label split; risks **R-11** (non-extended 0x53 would inject numpad `.` — a wrong-key bug that looks like success), **R-12** (held/double-tapped Del over-deletes; accepted as identical to a physical Delete key — `Del` stays under F-07 auto-repeat, not exempted), **R-13** (Row 4 asymmetry, accepted: rows are already independent star grids per D-13); test **T-19**. Numbering: A3 here; the word-prediction brief renumbers **A4**. | Approved — Nasser |
| 2026-07-27 | — | **Gate C0 approved** (A3 adopted) and **M11 implemented: selftest 208/208, 0 failed** (201 → 208: +2 Delete entries in the scan-code suite, +1 R-11 extended-flag check, +1 width, +1 Row 4 sequence lock, +1 cap-label check, +1 plain-key check). R-11 guard negative-tested: with `["Delete"] = new(0x53, false)` deliberately substituted, exactly the two expected checks failed; restoring the flag returned 208/208 — the extended marker cannot be lost silently. | Approved — Nasser |
| 2026-07-27 | — | **Gate C1 passed — T-19 witnessed.** The Approver ran the self-contained build (`1.2.0+5574e80`, sha `6820567f…7c15`) and confirmed the `Del` key works correctly in live use — the end-to-end R-11 check (a missing extended flag would have typed `.` instead of deleting). | Approved — Nasser |
| 2026-07-27 | — | **M12 complete — v1.2.1 package built; scope limited to a local commit.** `<Version>` 1.2.0 → 1.2.1 (the bump was required: a `Del`-bearing installer stamped `1.2.0` would be byte-different from the published v1.2.0 asset yet indistinguishable in Apps & Features, and would collide with an already-published release asset). `installer\build.ps1` run: exe `18fba8ec…9eb3` (FileVersion `1.2.1.0`, selftest 208/208 standalone), setup `01bba7ad…ccbf` (ProductVersion `1.2.1`). **No tag, no push, no GitHub Release** — the Approver restricted this step to a local commit; Gate C2 (release) remains open. | Approved — Nasser |
| 2026-07-27 | — | **Gate C2 passed — v1.2.1 released.** T-16 re-witnessed incidentally and cleanly: the Approver ran `softkeys-setup.exe` over the installed v1.2.0 — in-place upgrade succeeded, single Apps & Features entry now reading **1.2.1**, settings preserved; installed exe byte-identical to the release asset (`18fba8ec…9eb3`), confirming the released binary is the verified build. T-19 confirmed again on the installed copy. Tag `v1.2.1` (`c6d8631`) pushed to origin; GitHub Release published with both assets and notes per B3.8 (F-21, E0 53 rationale, 208/208, both hashes, upgrade guidance, SmartScreen/L-07 note, vboard credit). **GitHub-computed asset digests match the local hashes exactly** on both artifacts — verified via API, not assumed. `releases/latest` resolves to v1.2.1. | Approved — Nasser |
| 2026-07-27 | — | **Final acceptance v1.2.1.** Amendment A3 delivered in full: F-21 `Del` key plus the F-04 six-row correction, witnessed via T-19 and an incidental clean T-16 in-place upgrade; regression selftest 208/208 green with the R-11 extended-flag guard negative-tested. Input path unchanged — no new cross-process call, no new network code, no new timer. | **Signed off — Nasser** |
| 2026-07-27 | CR-21 | **Amendment A4 adopted — F-22 English word prediction; M13 authorized and implemented.** `docs/softkeys-v1.4-amendment.md` in force. Authored by the Implementer under the Approver's blanket grant of authority ("full authority to execute this feature in the most optimized way, based on your assessment"), which stands in place of a separate Gate D0 signature; the release gate (D2) remains the Approver's. Scope is completion only per the A4 brief: no correction, no learning, no Arabic prediction. Design decisions: **D-24** min prefix 2 / max 3 suggestions; **D-25** ordinally-sorted `string[]` + parallel `ushort[]` rank table with binary-search prefix range — **not** a trie (20 MB, slower) and **not** a DAWG (per-word ranks cannot survive minimisation); **D-26** word boundaries decided by the **emitted character**, not the active layout, which is what lets the strip stay out of the way under Arabic with **no layout detection and no NF-01 violation**; **D-27** Backspace/Delete end the tracked word rather than guessing a caret position; **D-28** Brotli-compressed embedded dictionary, committed blob + reproducible generator; **D-29** all Shift/CapsLock state read **before** injection and the emitted character resolved through one helper shared with `RefreshKeyCaps`; **D-30** accepting a suggestion bypasses `EmitKey` so it cannot start auto-repeat or feed itself back. **D-31/D4.4 dictionary provenance:** no third-party frequency data is used — `google-10000-english`, Norvig `count_1w` and SymSpell are all rejected, the last because its counts are byte-identical to Norvig's despite a CC-BY 3.0 claim (verified). Vocabulary is filtered against a public-domain (Unlicense) list and ranked in-project from Project Gutenberg prose. **L-08** records the accepted literary-frequency limitation. Risks **R-14** (stale buffer; append-only residual accepted) through **R-17**. F-10 amended: `Prediction` setting (default on, absent key deserializes to the default), default window height 380 → 414, minimum 260 → 300. Version target **v1.3.0**. Tests **T-20** (headless) / **T-21** (witnessed). Selftest grew 208 → 267+; engine green, dictionary asset build completing before Gate D1. | Approved — Nasser |
